...
def main(seed, model_name, cont=None, optim_name=None, subset=None, epoch=100):
    ...
    for epoch in range(total_epoch):  # loop over the dataset multiple times
        ...
        if ((epoch + 1) % 2 == 0) or (epoch + 1) == total_epoch:
            ...
            print("\tCNNU grads: ", gtv.cnnu.gtv.cnnu.layer[0].weight.grad.mean())
            pmax = list()
...
opt = OPT(batch_size = 50, admm_iter=4, prox_iter=3, delta=.1, channels=3, eta=.3, u=25, lr=8e-6, momentum=0.9, u_max=65, u_min=1e-2)
